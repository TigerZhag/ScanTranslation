...
inline bool TBOX::major_x_overlap(const TBOX &box) const {
  ...
  if(IsChineseOCR)
  {
	  ...
	  {
		  LOGE("box.left() - this->right()=%f",(float)box.left() - this->right());
		  LOGE("this->width()/%d=%f",Scale,(float)this->width()/Scale);
		  LOGE("(float)box.width()/%d=%f",Scale,(float)box.width()/Scale);
	  }else if(this->left() > box.right())
	  {
		  
		  LOGE("(float)this->left() - box.right()=%f",(float)this->left() - box.right());
		  LOGE("this->width()/%d=%f",Scale,(float)this->width()/Scale);
		  LOGE("(float)box.width()/%d=%f",Scale,(float)box.width()/Scale);
	  }

	  LOGE("IsChineseOCR  TRUE!");
	  if (this->left() > box.left()&&this->left()<box.right())
	  {
          lbMerge=true;
		  LOGE("this->left() > box.left()&&this->left()<box.right()=true");
	  }else if (this->right() > box.left()&&this->right()<box.right())
	  {
          lbMerge=true;
		  LOGE("this->right() > box.left()&&this->right()<box.right()=true");
	  }else if (this->left() > box.left()&&this->right()<box.right())
	  {
          lbMerge=true;
		  LOGE("this->left() > box.left()&&this->right()<box.right()=true");
	  }else if(this->left() > box.right()&&
		      (((float)this->left() - box.right()<(float)this->width()/Scale)  ||  ((float)this->left() - box.right()<(float)box.width()/Scale)   ))
	  {
		  lbMerge=true;
		  LOGE("this->left() > box.right()&&");
	  }else if(this->right() < box.left()&&
		      (((float)box.left() - this->right()<(float)this->width()/Scale)  ||  ((float)box.left() - this->right()<(float)box.width()/Scale)   ))
	  {
		  lbMerge=true;
		  LOGE("this->right() < box.left()&&");
	  }
  }*/
