...
void WERD_RES::SetupBlobWidthsAndGaps() {
  ...
  for (int b = 0; b < num_blobs; ++b) {
	...
	blob_heights.push_back(box.height());
    if (b + 1 < num_blobs) {
      blob_gaps.push_back(
...
// Returns the the height of the blob
// inclusive.
int WERD_RES::GetBlobsHeight(int blobindex) {
  int result = 0;
  result += blob_heights[blobindex];
  return result;
}
