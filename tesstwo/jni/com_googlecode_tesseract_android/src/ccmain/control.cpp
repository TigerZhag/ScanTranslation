...
#define LANGUAGECORR  -4.3
#define OCRWORDCOUNT  100
...
bool Tesseract::RecogAllWordsPassN(int pass_n, ETEXT_DESC* monitor,
  ...
  int liRecogWordCount=0;
  float TotalAverageCertainty=0.0;
  ...
  for (int w = 0; w < words->size(); ++w) {
	...
	TempCoordinate1.min_x=word->word->word->bounding_box().left();
	TempCoordinate1.min_y=word->word->word->bounding_box().bottom();
	TempCoordinate1.max_x=word->word->word->bounding_box().right();
	TempCoordinate1.max_y=word->word->word->bounding_box().top();
	moVectorPreCoordinate.push_back(TempCoordinate1);*/
	
	int liTooSmallBlockCounter=0;
	int liNomalBlockCounter=0;
	C_BLOB_LIST *list = word->word->word->cblob_list();
	...
	for (c_blob_it.mark_cycle_pt();!c_blob_it.cycled_list();c_blob_it.forward()) 
	{
		 ...
		 if(c_blob->bounding_box().right()-c_blob->bounding_box().left()<5||
	        c_blob->bounding_box().top()-c_blob->bounding_box().bottom()<5)
		 {
			 liTooSmallBlockCounter++;
		 }else{
		     liNomalBlockCounter++;
		 }
	}
	...
	if(liTooSmallBlockCounter>liNomalBlockCounter)
	{
		word->word->SetupFake(unicharset);
	}
    if (word->word->tess_failed) continue;
	
   /* struct CornerCoordinate TempCoordinate;
	TempCoordinate.min_x=word->word->word->bounding_box().left();
	TempCoordinate.min_y=word->word->word->bounding_box().bottom();
	TempCoordinate.max_x=word->word->word->bounding_box().right();
	TempCoordinate.max_y=word->word->word->bounding_box().top();
	moVectorCoordinate.push_back(TempCoordinate);
	*/
...
/*		for(int j=0;j<word->word->box_word->length();++j)
		{
			struct CornerCoordinate TempCoordinate;
			TempCoordinate.min_x=word->word->box_word->BlobBox(j).left();
			TempCoordinate.min_y=word->word->box_word->BlobBox(j).bottom();
			TempCoordinate.max_x=word->word->box_word->BlobBox(j).right();
			TempCoordinate.max_y=word->word->box_word->BlobBox(j).top();
			TempCoordinate.AverageCertainty=0;
			moVectorPreCoordinate.push_back(TempCoordinate);
		}
		*/

	int WordLength=word->word->best_choice->length();
	float AverageCertainty=0.0;
	//LOGD("TotalOfStates=%d",word->word->best_choice->TotalOfStates());
	for(int i=0;i<WordLength;++i)
	{		
		AverageCertainty+=word->word->best_choice->certainty(i);
		TotalAverageCertainty+=word->word->best_choice->certainty(i);
		if(classify_debug_level||1)
		{
			LOGD("best_choice->GetUniChar(I)=%s   C:%.4g",word->word->best_choice->GetUniChar(i).c_str(),word->word->best_choice->certainty(i));
		}
	}
	
	AverageCertainty =AverageCertainty/(float)WordLength;
	//LOGD("Average=%.4g",AverageCertainty);

	int i=0;
	if(word->word->best_choice->certainty(0)<-5.8)
	{
		for(i=0;i<WordLength;++i)
		{ 
			word->word->best_choice->remove_unichar_ids(i,1);
			word->word->best_choice->append_unichar_id_space_allocated(UNICHAR_SPACE,1,0.0f, 0.0f);
			--WordLength;
			--i;
		}
	}
	
	liRecogWordCount+=WordLength;
	if(IsShutDown||liRecogWordCount>OCRWORDCOUNT)
	{  
		//LOGD("liRecogWordCount=%d",liRecogWordCount);
		for (int cur = w+1; cur < words->size(); ++cur) {
           WordData* word = &(*words)[cur];
		   word->word->SetupFake(unicharset);
		}
		TotalAverageCertainty=TotalAverageCertainty/(float)liRecogWordCount;
		if(IsShutDown)
		{
			LOGI("in RecogAllWordsPassN IsShutDown");
			mbLangCorre=true;
			return false;
		}else
		{
			//LOGI("in RecogAllWordsPassN TotalAverageCertainty=%f",TotalAverageCertainty);
			mbLangCorre=TotalAverageCertainty>LANGUAGECORR?true:false;
			return true;
		}
	}
	...
	/*for(int i=0;i<WordLength;++i)
	{
		LOGD("word->word->best_choice->GetUniChar(I)=%s   C:%.4g",word->word->best_choice->GetUniChar(i).c_str(),word->word->best_choice->certainty(i));
		if(word->word->best_choice->certainty(i)<-5.0)
		{
			word->word->best_choice->remove_unichar_ids(i,1);
            word->word->best_choice->append_unichar_id_space_allocated(UNICHAR_SPACE,1,0.0f, 0.0f);
			--WordLength;
  ...
  TotalAverageCertainty=TotalAverageCertainty/(float)liRecogWordCount;
  //LOGD("TotalAverageCertainty=%f",TotalAverageCertainty);
  mbLangCorre=TotalAverageCertainty>LANGUAGECORR?true:false;
  return true;
}
...
bool Tesseract::recog_all_words(PAGE_RES* page_res,
  ...
  */
  ...
  // ****************** Pass 2 *******************
  if (tessedit_tess_adaption_mode != 0x0 &&0  && !tessedit_test_adaption) {
    page_res_it.restart_page();
    GenericVector<WordData> words;
  ...
  if (tessedit_ocr_engine_mode == OEM_TESSERACT_ONLY) {
    // ****************** Pass 3 *******************（修复模糊，残影）
    // Fix fuzzy spaces.
	  LOGD(" if (tessedit_ocr_engine_mode == OEM_TESSERACT_ONLY )");
    set_global_loc_code(LOC_FUZZY_SPACE);

    if (!tessedit_test_adaption &&0&& tessedit_fix_fuzzy_spaces
...
static bool NewWordBetter(const WERD_RES& word, const WERD_RES& new_word,
  ...
  /**
  modified by zhangdj  ,use Average Certainty instead of sum Certainty
  **/
    int NewWordLength=new_word.best_choice->length();
  	int WordLength=word.best_choice->length();

	float NewAverageCertainty=0.0;
	float AverageCertainty=0.0;
	for(int i=0;i<WordLength;++i)
	{
		//LOGD("word.best_choice->GetUniChar(I)=%s   C:%.4g",word.best_choice->GetUniChar(i).c_str(),word.best_choice->certainty(i));
		AverageCertainty+=word.best_choice->certainty(i);
	}
	for(int i=0;i<NewWordLength;++i)
	{
		//LOGD("    new_word.=%s   C:%.4g",new_word.best_choice->GetUniChar(i).c_str(),new_word.best_choice->certainty(i));
		NewAverageCertainty+=new_word.best_choice->certainty(i);
	}
	
    NewAverageCertainty =NewAverageCertainty/(float)NewWordLength;
    AverageCertainty =AverageCertainty/(float)WordLength;
   
	//LOGD("AverageCertainty=%f  rating=%f",AverageCertainty,word.best_choice->rating());
	//LOGD("NewAverageCertainty=%f rating=%f",NewAverageCertainty,new_word.best_choice->rating());

  if (NewAverageCertainty > AverageCertainty&&
      new_word.best_choice->rating() < word.best_choice->rating()*10/6) {
...
void Tesseract::classify_word_and_language(WordRecognizer recognizer,
  ...
  if(miLanguage==ENG_ONLY)
  {
	 most_recently_used_ = sub_langs_[1];
  }else
  {
	 most_recently_used_ = this;
  }
  ...
  if (initially_done) {
	...
	if(strcmp(most_recently_used_->lang.string(),"libchi_sim")==0||strcmp(most_recently_used_->lang.string(),"libpinyin")==0)
	{
		mbIsChi=true;
	}else
	{
	...
	//LOGD("OCR WITH %s",most_recently_used_->lang.string());
	/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
  ...
  if (word->tess_failed || !word->tess_accepted) {
   ...
   /* if (most_recently_used_ != this) {
      ...
      if (word_data->word->tesseract == this) {
		...
		if(strcmp(this->lang.string(),"libchi_sim")==0)
		{
			mbIsChi=true;
		}else
		{
	...
	*/
    for (int i = 0; i < sub_langs_.size(); ++i) {
      if (sub_langs_[i] != previous_used) {
		...
		if(strcmp(sub_langs_[i]->lang.string(),"libnum")==0&&miLanguage==CHI_ONLY)
		{
			continue;
		}
		
		if((strcmp(sub_langs_[i]->lang.string(),"libchi_sim")==0||strcmp(sub_langs_[i]->lang.string(),"libpinyin")==0)&&miLanguage==ENG_ONLY)
		{
			continue;
		}

		if(strcmp(sub_langs_[i]->lang.string(),"libchi_sim")==0||strcmp(sub_langs_[i]->lang.string(),"libpinyin")==0)
		{
			mbIsChi=true;
		}else
		{
		...
		LOGD("OCR WITH %s",sub_langs_[i]->lang.string());
