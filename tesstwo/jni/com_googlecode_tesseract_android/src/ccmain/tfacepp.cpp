...
#define MAX_UNDIVIDED_LENGTH 40
...
void Tesseract::recog_word(WERD_RES *word) {
	//LOGE("1111111111word->box_word->length()=%d",word->box_word->length());
  ...
  word->SetupBoxWord();//make box_word
...
void Tesseract::recog_word_recursive(WERD_RES *word) {
  ...
  if (word_length > MAX_UNDIVIDED_LENGTH) {
	LOGE("split_and_recog_word(word_length=%d)",word_length);
