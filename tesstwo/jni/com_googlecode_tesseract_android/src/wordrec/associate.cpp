...
const float AssociateUtils::kMinGap = 0.025f;
...
void AssociateUtils::ComputeStats(int col, int row,
  ...
  /** modified by zhangdj because the  blob_row->x_height()and blob_row->ascenders() is not stable**/
  normalizing_height=(word_res->bln_boxes->bounding_box().top()-word_res->bln_boxes->bounding_box().bottom())*word_res->denorm.y_scale();
  ...
  if (fixed_pitch) {
    ...
    if (col > 0) {
      ...
      if ((!end_row && left_gap < kMinGap) || left_seam->priority > 0.0f) {
		...
		//LOGD("left_gap=%f,kMinGap=%f   %s",left_gap,kMinGap,left_gap < kMinGap ? "left_gap < kMinGap" : "");
      if (debug) {
    ...
    if (!end_row) {
      ...
      if (right_gap < kMinGap || right_seam->priority > 0.0f) {
		...
		//LOGD("right_gap=%f,kMinGap=%f   %s",right_gap,kMinGap,right_gap < kMinGap ? "right_gap < kMinGap" : "");
      if (debug) {
