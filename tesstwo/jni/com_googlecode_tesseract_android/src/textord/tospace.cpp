...
BOOL8 Textord::isolated_row_stats(TO_ROW *row,
                                  ...
                                  inT16 block_idx,
                                  inT16 row_idx) {
  //LOGE("Textord::isolated_row_stats");
  float kern_estimate;
  ...
  if (!box_it.empty ()) {
    ...
    do {
      ...
      if (!bblob->joined_to_prev() && bblob->cblob() != NULL) {
		...
		/*LOGE("bblob height %f",(float)bblob->bounding_box().top()-(float)bblob->bounding_box().bottom());
		LOGE("row*0.5 height %f",0.5*((float)row->max_y()-(float)row->min_y()));
		LOGE("bblob width %f",(float)bblob->bounding_box().right()-(float)bblob->bounding_box().left());
		LOGE("bblob apart row bottom %f",(float)bblob->bounding_box().bottom()-(float)row->min_y());
		
		if((((float)bblob->bounding_box().top()-(float)bblob->bounding_box().bottom()<0.4*((float)row->max_y()-(float)row->min_y()))&&((float)bblob->bounding_box().right()-(float)bblob->bounding_box().left()<1.1*((float)bblob->bounding_box().top()-(float)bblob->bounding_box().bottom()))&&((float)bblob->bounding_box().top()-(float)row->min_y()<0.5*((float)row->max_y()-(float)row->min_y()))))
		{
			LOGE("is a dou  hao !!!!!!!!!!!!!!");
			LOGE("bblob height %f",(float)bblob->bounding_box().top()-(float)bblob->bounding_box().bottom());
			LOGE("row*0.5 height %f",0.5*((float)row->max_y()-(float)row->min_y()));
			LOGE("bblob width %f",(float)bblob->bounding_box().right()-(float)bblob->bounding_box().left());
			LOGE("bblob top apart row bottom %f",(float)bblob->bounding_box().top()-(float)row->min_y());
		}*/
        ...
        if (blob_box.left () > next_rep_char_word_right ||
            make_a_word_break(row, blob_box, prev_gap_arg, prev_blob_box,
                              ...
                              break_at_next_gap)||
			((((float)bblob->bounding_box().top()-(float)bblob->bounding_box().bottom()<0.3*((float)row->max_y()-(float)row->min_y()))&&((float)bblob->bounding_box().right()-(float)bblob->bounding_box().left()<1.1*((float)bblob->bounding_box().top()-(float)bblob->bounding_box().bottom()))&&((float)bblob->bounding_box().top()-(float)row->min_y()<0.4*((float)row->max_y()-(float)row->min_y()))))||
            box_it.at_first()) {
...
BOOL8 Textord::make_a_word_break(
  ...
  */
  ...
  {
	LOGE("current_gap = within_xht_current_gap;");
  ...
  if (tosp_old_to_method||1) {
                                 //Boring old method
	//LOGE("row->max_nonspace=%d(%d) row->space_threshold*3/2(%d)  row->min_space*3/2(%d)",row->max_nonspace,row->max_nonspace*4/5,row->space_threshold*3/2,row->min_space*3/2);

	//LOGE("current_gap=%d",current_gap);
    space = (float)current_gap > (((float)row->max_nonspace)*80/100);
	...
	//LOGE("blanks(%d)  current_gap(%d) max_nonspace*80/100(%f) space_threshold(%d) min_space(%d) space_size(%f)",blanks,current_gap,((float)row->max_nonspace)*80/100,row->space_threshold,row->min_space,row->space_size);
    return space;
  }
  else {
  /* New exciting heuristic method */
    ...
    */
    ...
    if (tosp_debug_level > 10)
	{
      tprintf("row->max_nonspace=%d word break = %d current_gap = %d, prev_gap = %d, "
              "next_gap = %d\n", row->max_nonspace,space ? 1 : 0, current_gap,
              prev_gap, next_gap);
	}
