...
static int increseer=0;
static int paramincreseer=0;

struct DividingBLOCK
{
	int  LineDividingLeft;
	int  LineDividingRight;
	int  LineDividingTop;
	int  LineDividingBottom;
	int  RowTop;
	int  RowBottom;
	int  BLOCKWidth;
	int  BLOCKHeight;
...
class TESS_API TessBaseAPI {
 public:
  ...
  vector<IplImage*>    moVectorIplImage;
  bool           baseapi_debug;

  vector<YBLOCK> moVectorYBLOCK;
  vector<DividingBLOCK> moVectorDivBLOCK;
  vector<DividingBLOCK> moVectorLeftChar;
  vector<DividingBLOCK> moVectorNumberChar;
  vector<DividingBLOCK> moVectorRow;
  vector<DividingBLOCK> moVectorAll;
  vector<DividingBLOCK> moVectorBetweenStartAndSec;
  DividingBLOCK         moTargetLeave;

  
  vector<DividingBLOCK> moVectorAutoRectResult;
  //vector<DividingBLOCK> moVectorApplyBLOCK;
  /**
  ...
  int Init(const char* datapath, const char* language, const char* suffix,OcrEngineMode oem) {
    return Init(datapath, language,suffix, oem, NULL, 0, NULL, NULL, false);
  }
  int Init(const char* datapath, const char* language ,const char* suffix) {
    return Init(datapath, language,suffix, OEM_DEFAULT, NULL, 0, NULL, NULL, false);
  }
   ...
   */
  void SetImageForFindLines(const Pix* pix);

  /**
   * set image for find lines to select left words
   */
  void SetImageForFindLeft(uchar *src,int aiWidth,int aiHeight,int aiChannels,int aiRGBDisThreshold);
  ...
  */
   ...
   * as UTF8 and must be freed with the delete [] operator.
   */
  char* GetUTF8TextWith(int aiLanguage);


  void  SetLanguage(int aiLanguage);


  bool  GetISLangCorre();
  /**Get pixs from TargetImg in the area that regular by SourceImg**/
  int GetAreaInImage(unsigned char* SourceImageData,
					unsigned char* TargetImageData,
                    int width, int height,
                    int bytes_per_pixel);
  int GetBlockInApply(unsigned char* SourceImageData,
					unsigned char* TargetImageData,
					int width, int height,
					int bytes_per_pixel);

  bool GetAreaState(unsigned char* SourceImageData,
                                int width, int height,
                                int bytes_per_pixel);

  /**Get Blocks in image that make by regconize process**/
  int GetImageWithBlocks(unsigned char* ImageData,
                         int width, int height,
						 int bytes_per_pixel);

  /**Remove Noise Pix from image**/
  int RemoveNoisePix(unsigned char* SourceImageData,
					unsigned char* TargetImageData,
					unsigned char* ReturnImageData,
                    int width, int height,
                    int bytes_per_pixel);
  
  int Tidy(unsigned char* ImageData,
			int width, int height,
			int bytes_per_pixel);

  /**if photo too small**/
  int IfPhotoTooSmall(int aiBlobHigMin,int aiBlobHigMax,float afCertaintyThre);
  /**
  ...
  void InitBreak();
	...
	int       Threshold(uchar *src,int aiWidth,int aiHeight,int aiChannels,const char *apcFileName);
	static int  AdaptiveThreshold(uchar *src,int aiWidth,int aiHeight,int aiChannels,const char *apcFileName,int aiSrcHeight);
	IplImage* AdaptiveThresholdAndBlank(uchar *src,int aiWidth,int aiHeight,int aiChannels,int aiRGBDisThreshold);
	uchar     *GetAdaptiveThresholdImg(uchar *src,int aiWidth,int aiHeight,int aiChannels,int &aiWidthStep);
	uchar     *GetAdaptiveThresholdByte(uchar *src,int aiWidth,int aiHeight,int aiChannels,int &aiImgSize);
	bool      NeedToFocus(uchar *src,int aiWidth,int aiHeight,int aiChannels);
	void      OnYcbcrY(uchar *src,int aiWidth,int aiHeight,int aiChannels);
	int       ImageStretchByHistogram(IplImage *src1,IplImage *dst1);
	int       ImageBlackCount(IplImage *src1);
	int       PixaLocate(IplImage *src,int aiCountSum)  ;
  /**
 ...
 protected:
  Mat               *mpoLastFoucusMat;
  IplImage          *mpoLastFoucusIpl;
  vector<KeyPoint>  moLastkeyPoints;  
  Mat               moLastdescriptors;  
 ...
 private:
  ...
  int DrawImageWithBlocks(unsigned char* ImageData,
									int width, int height,
									int bytes_per_pixel,
  ...
  int FindDividingLine(unsigned char* ImageData,
					int width, int height,
					int bytes_per_pixel);
  
  int FindXDividingLine(unsigned char* ImageData,
						int width, int height,
						int bytes_per_pixel,
						struct YBLOCK &yblock);
  int FindYDividingLine(unsigned char* ImageData,
						int width, int height,
						int bytes_per_pixel,
						struct XBLOCK &xblock);
  float ComputeAphaPercent(unsigned char* ImageData,
							int width, int height,
							int bytes_per_pixel,
							int top,int bottom,int left,int right);
  int BlankIt(unsigned char* ImageData,
				int width, int height,
				int bytes_per_pixel,
				int top,int bottom,int left,int right);
  int BlankIt(unsigned char* ImageData,
...
DividingBLOCK FindPartToBlank(unsigned char* ImageData,
							int width, int height,
							int bytes_per_pixel,
							int bytes_per_line);

uchar * RemoveColorBlock(unsigned char* ImageData,
						int width, int height,
					    int bytes_per_pixel,
						int bytes_per_line,
...
int Fuzzy_or_Clear_Judge(IplImage *src,int thresholdnum,int threshold1,int blocksize);
 ...
 double  focus(IplImage *tar);

  int SelectIt(unsigned char* ReturnImageData,
				unsigned char* TargetImageData,
				int width, int height,
				int bytes_per_pixel,
				int top,int bottom,int left,int right);
  int ColorIt(unsigned char* ImageData,
				int width, int height,
				int bytes_per_pixel,
				int top,int bottom,int left,int right);
  void light(unsigned char &ImageData,int offect);
  void deep(unsigned char &ImageData,int offect);
  void DeepClolor(unsigned char &ImageData,int Target,int Offect);
  void DeepToClolor(unsigned char &ImageData,int Target);
  void AdaptoClolor(unsigned char &ImageData,int Target,int Offect);
  int IfHeightLine(unsigned char* ImageData,
				   int width, int height,
					int bytes_per_pixel,
					struct XBLOCK xblock);
};  // class TessBaseAPI.
