...
public class TessBaseAPI {
    ...
    public boolean init(String datapath, String language) {
        return init( datapath,  language, "so");
    }

    public boolean init(String datapath, String language, String suffix) {
        setDebug(false);
        if (datapath == null)
        ...
        if (!tessdata.exists() || !tessdata.isDirectory()) {
            Log.e("zhangdj",datapath+" !tessdata.exists() || !tessdata.isDirectory()");
            throw new IllegalArgumentException("Data path must contain subfolder tessdata!");
        }

        return nativeInit(datapath, language,  suffix);
    }
     ...
     * @return <code>true</code> on success
     */
    public boolean init(String datapath, String language, String suffix, int ocrEngineMode) {
        if (datapath == null)
            throw new IllegalArgumentException("Data path must not be null!");
        if (!datapath.endsWith(File.separator))
            datapath += File.separator;
        ...
        return nativeInitOem(datapath, language,suffix, ocrEngineMode);
    }
    ...
    /**
     * set image for find lines to select words
     */
    public void setImageForFindLines(Bitmap bmp) {
        Pix image = ReadFile.readBitmap(bmp);
        ...
        nativeSetImageForFindLines(image.getNativePix());
    }
    ...
    /**
     * @param bmp used ocr
     * @param RGBDisThreshold Threshold for ocr
     * set image for find lines to select left words
     */
    public int[] setImageForFindLeft(Bitmap bmp,int RGBDisThreshold) {
        return nativeSetImageForFindLeft(bmp,RGBDisThreshold);
    }

    public Bitmap bytes2Bimap(byte[] b, int w, int h) {
        int[] ints = new int[b.length / 4];
        for (int i = 0, j = 0; j < ints.length; i += 4, j++) {
            ints[j] = Color.argb(b[i + 3] & 0xff, b[i] & 0xff, b[i + 1] & 0xff, b[i + 2] & 0xff);
        }
     ...
     *
     * @param image Leptonica pix representation of the imagenativeSetImageBytes
     */
    ...
    public void setImage(byte[] imagedata, int width, int height, int bpp) {
        nativeSetImageBytes(imagedata, width, height, bpp);
    }
     ...
     * @return the recognized text
     */
    public String getUTF8TextWith(int language) {
        // Trim because the text will have extra line breaks at the end
        String text = nativeGetUTF8TextWith(language);
        if (text==null)
        {
            return null;
        }
    ...
    public boolean getISLangCorre() {
        return  nativeGetISLangCorre();
    }

    /**
     *Get pixs from TargetImg in the area that regular by SourceImg
     *
     * @param SrcImgData byte representation of the Source image that regular the area
     * @param TarImgData byte representation of the Target image
     * @param width image width ,target's is the equal to source's
     * @param height image height ,target's is the equal to source's
     * @param bpp bytes per pixel
     */
    public byte[] getAreaInImage(byte[] SrcImgData, byte[] TarImgData,
                                 int width, int height, int bpp){

        return nativeGetAreaInImage(SrcImgData,TarImgData,width,height,bpp);
    }

    public byte[] getBlockInApply(byte[] SrcImgData, byte[] TarImgData,
                                  int width, int height, int bpp){

        return nativeGetBlockInApply(SrcImgData,TarImgData,width,height,bpp);
    }

    public byte[] getImageTest(){

        return nativeGetImageTest();
    }

    public int getImageTestWidth(){

        return nativeGetImageTestWidth();
    }
    public int getImageTestHeight(){

        return nativeGetImageTestHeight();
    }
	
    /**
     *Get Blocks in image that make by regconize process
     *
     * @param Data byte representation of the Source image
     * @param width image width 
     * @param height image height
     * @param bpp bytes per pixel
     */
    public byte[] getImageWithBlocks(byte[] Data, int width, int height, int bpp){

        return nativeGetImageWithBlocks(Data,width,height,bpp);
    }

    /**
     *Remove  Noise  Pix  from image
     *
     * @param SrcImgData byte representation of the Source image that regular the area
     * @param TarImgData byte representation of the Target image
     * @param width image width 
     ...
     * @param bpp bytes per pixel
     */
    public byte[] removeNoisePix(byte[] SrcImgData, byte[] TarImgData,
                                 int width, int height, int bpp){

        return nativeRemoveNoisePix(SrcImgData,TarImgData,width,height,bpp);
    }


    public Bitmap tidy(Bitmap bmp){
        ByteBuffer BottomBuffer = ByteBuffer.allocate(bmp.getByteCount());
        bmp.copyPixelsToBuffer(BottomBuffer);
        byte[] result =nativeTidy(BottomBuffer.array(),bmp.getWidth(),bmp.getHeight(),4);
        return bytes2Bimap(result, bmp.getWidth(), bmp.getHeight());
    }
	
    /**
     *Get Blocks in image that make by regconize process
     *
     * @param BlobHigMin threshold of Blobs Hight
     * @param BlobHigMax  threshold of Certainty
     */
    public int IfPhotoTooSmall(int BlobHigMin, int BlobHigMax){

        return nativeIfPhotoTooSmall(BlobHigMin,BlobHigMax);
    }
    ...
    private native boolean nativeInit(String datapath, String language, String suffix);

    private native boolean nativeInitOem(String datapath, String language, String suffix, int mode);
    ...
    private native void nativeSetImageBytes(
            byte[] imagedata, int width, int height, int bpp);
			
    private native void nativeSetImagePix(int nativePix);

    private native void nativeSetImageForFindLines(int nativePix);

    private native  int[] nativeSetImageForFindLeft(Bitmap bmp,int RGBDisThreshold);

    private native void nativeSetRectangle(int left, int top, int width, int height);

    private native String nativeGetUTF8Text();

    private native String nativeGetUTF8TextWith(int language);

    private native boolean nativeGetISLangCorre();
	
    private native byte[] nativeGetAreaInImage(byte[] SrcImgData,byte[] TarImgData,
                                               int width, int height,int bpp);

    private native byte[] nativeGetBlockInApply(byte[] SrcImgData,byte[] TarImgData,
                                                int width, int height,int bpp);
    private native byte[] nativeGetImageTest();
    private native int nativeGetImageTestHeight();
    private native int nativeGetImageTestWidth();


    private native byte[] nativeGetImageWithBlocks(byte[] Data,int width, int height,int bpp);
	
    private native byte[] nativeRemoveNoisePix(byte[] SrcImgData,byte[] TarImgData,
                                               int width, int height,int bpp);

    private native byte[] nativeTidy(byte[] TarImgData,int width, int height,int bpp);

    private native int nativeIfPhotoTooSmall(int BlobHigMin, int BlobHigMax);

    private native void nativeStop();
    
    private native void nativeInitBreak();
    ...
    private native int nativeThreshold(byte[] src, int aiWidth, int aiHeight, int aiChannels,String FileName);

    private native int nativeAdaptiveThreshold(Bitmap bitmap,String FileName,int srcHeight);

    private native int nativeAdaptiveThresholdAndBlank(byte[] src, int aiWidth, int aiHeight, int aiChannels,String FileName);

    private native byte[] nativeGetAdaptiveThresholdImg(byte[] src, int aiWidth, int aiHeight, int aiChannels);

    private native byte[] nativeGetAdaptiveThresholdByte(byte[] src, int aiWidth, int aiHeight, int aiChannels);

    public int threshold(byte[] src, int aiWidth, int aiHeight, int aiChannels, String FileName) {
        return nativeThreshold(src, aiWidth, aiHeight, aiChannels,FileName);
    }

    public int adaptiveThreshold(Bitmap bitmap, String FileName, int srcHeight) {
        return nativeAdaptiveThreshold(bitmap,FileName,srcHeight);
    }

    public int adaptiveThresholdAndBlank(byte[] src, int aiWidth, int aiHeight, int aiChannels, String FileName) {
        return nativeAdaptiveThresholdAndBlank(src, aiWidth, aiHeight, aiChannels,FileName);
    }

    public byte[] getAdaptiveThresholdImg(byte[] src, int aiWidth, int aiHeight, int aiChannels) {
        return nativeGetAdaptiveThresholdImg(src, aiWidth, aiHeight, aiChannels);
    }

    public byte[] getAdaptiveThresholdByte(byte[] src, int aiWidth, int aiHeight, int aiChannels) {
        return nativeGetAdaptiveThresholdByte(src, aiWidth, aiHeight, aiChannels);
    }
